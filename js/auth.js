...
function iniciarComAutenticacao(){
  ...
  firebase.auth().onAuthStateChanged(user=>{
    if(user){
      ...
      AVISOS = []; NOTICIAS_MANUAIS = []; IMAGENS = []; CAMPANHAS = []; TABELA = [];
      [CHAVES.config, CHAVES.avisos, CHAVES.noticias, CHAVES.imagens, CHAVES.campanhas, CHAVES.tabela].forEach(k=>{ try{ localStorage.removeItem(k); }catch{} });
